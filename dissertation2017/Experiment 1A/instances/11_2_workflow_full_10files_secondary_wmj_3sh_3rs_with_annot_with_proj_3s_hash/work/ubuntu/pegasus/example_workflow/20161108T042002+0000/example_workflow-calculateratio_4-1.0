...
##################################################################
import ConfigDB_Calc_StatsCPUMemory_4
client_statscpumemory = DataStoreClient("mongodb", ConfigDB_Calc_StatsCPUMemory_4)
...
##################################################################
import ConfigDB_Calc_TEInfo_4
client_taskinfo = DataStoreClient("mongodb", ConfigDB_Calc_TEInfo_4)
...
if(eventList):
    for index in eventList:
        ...
        event_type = index[ConfigDB_Calc_TEInfo_4.COLUMN]
        while True:
            doc = index['data'].next()
            if doc is None:
                break;
...
import ConfigDB_Calc_TaskEvent_4
client_task = DataStoreClient("mongodb", ConfigDB_Calc_TaskEvent_4)
