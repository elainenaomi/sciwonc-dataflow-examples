...
import sys
##################################################################
import ConfigDB_Task_StatsCPUMemory_1
client_averagecpu = DataStoreClient("postgres", ConfigDB_Task_StatsCPUMemory_1)
...
if data_averagecpu:
    while True:
        ...
        sd_cpu = float(doc['standard deviation cpu'])
        avg_cpu = float(doc['average cpu'])
        sd_memory = float(doc['standard deviation memory'])
        avg_memory = float(doc['average memory'])
        sd_ratio = float(doc['standard deviation ratio'])
        avg_ratio = float(doc['average ratio'])


##################################################################
import ConfigDB_Task_GeneralInfo_1
client_geninfo = DataStoreClient("postgres", ConfigDB_Task_GeneralInfo_1)
...
if data_geninfo:
    while True:
        ...
        if doc is None:
            break;
        print doc
        total_items = float(doc['total items'])

##################################################################


import ConfigDB_TaskEvent_1
import math

# connector and config
config = ConfigDB_TaskEvent_1
client = DataStoreClient("postgres", config)
