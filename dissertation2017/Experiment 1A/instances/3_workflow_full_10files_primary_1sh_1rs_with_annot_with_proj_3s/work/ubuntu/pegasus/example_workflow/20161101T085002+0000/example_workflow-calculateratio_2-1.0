...
##################################################################
import ConfigDB_Calc_StatsCPUMemory_2
client_statscpumemory = DataStoreClient("postgres", ConfigDB_Calc_StatsCPUMemory_2)
...
##################################################################
import ConfigDB_Calc_TEInfo_2
client_taskinfo = DataStoreClient("postgres", ConfigDB_Calc_TEInfo_2)
...
if(eventList):
    for index in eventList:
        ...
        event_type = index[ConfigDB_Calc_TEInfo_2.COLUMN]
        while True:
            doc = index['data'].next()
            if doc is None:
                break;
...
import ConfigDB_Calc_TaskEvent_2
client_task = DataStoreClient("postgres", ConfigDB_Calc_TaskEvent_2)
