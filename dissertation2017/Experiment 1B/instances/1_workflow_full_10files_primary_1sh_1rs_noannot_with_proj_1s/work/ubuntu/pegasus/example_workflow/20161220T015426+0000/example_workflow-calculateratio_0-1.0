...
##################################################################
import ConfigDB_Calc_TaskEventInfo_0
client_taskinfo = DataStoreClient("postgres", ConfigDB_Calc_TaskEventInfo_0)
...
if(eventList):
    for index in eventList:
        ...
        event_type = index[ConfigDB_Calc_TaskEventInfo_0.COLUMN]
        while True:
            doc = index['data'].next()
            if doc is None:
                break;
