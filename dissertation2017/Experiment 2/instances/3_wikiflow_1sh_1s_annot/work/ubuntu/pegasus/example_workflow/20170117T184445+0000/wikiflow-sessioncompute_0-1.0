...
import ConfigDB_SessionCompute_0
import pprint

# connector and config
client = DataStoreClient("postgres", ConfigDB_SessionCompute_0)
config = ConfigDB_SessionCompute_0
...
if(dataList):
    for i in dataList:
        ...
        if contributor_username:
            ...
            print "saving"
            clientOutput = DataStoreClient("postgres", ConfigDB_SessionCompute_0)
            clientOutput.saveData(output, numline=numline)
            numline += len(output)
