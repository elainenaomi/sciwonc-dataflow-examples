...
client = DataStoreClient("postgres", ConfigDB_UserGroup_0)
...
if(data):
    ...
    while True:
        ...
        if doc is None:
            break;
        print doc
        if doc["contributor_username"]:        
            contributor_username =  doc["contributor_username"]
            contributors.append(contributor_username)
            output.append({"contributor_username": contributor_username})

    if output:
        clientOutput = DataStoreClient("postgres", ConfigDB_UserGroup_0)
