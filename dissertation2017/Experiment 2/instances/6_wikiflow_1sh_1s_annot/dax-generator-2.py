...
with open(PATH + '/config.json') as configFile:
    ...
    for task in configData["tasks"]:
        ...
        if totalTasks > 0:
            ...
            # if task.has_key("outputPrefixFile"):
    ...
    if totalJobs > 0:
        ...
        for numTaskIt in range(0, totalJobs):
            ...
            for name in exeKeylist:
                ...
                if (configData["tasks"][numTaskIt].has_key("outputPrefixFile")):

                    totalSlaves = configData["tasks"][numTaskIt]["totalSlaves"]
                    for i in range(0, totalSlaves):
                        file_template = configData["tasks"][numTaskIt]["outputPrefixFile"]
                        file_template += "_" + str(i) + ".py"

                        outputFile = File(file_template)
                        jobTask[jobTaskIt].uses(outputFile, link=Link.OUTPUT)

                if (configData["tasks"][numTaskIt].has_key("dynamicInputFile")):
                    dynInpIt = int(name.split(".")[0].split("_")[-1])
                    file_template = configData["tasks"][numTaskIt]["dynamicInputFile"]
                    file_template += "_" + str(dynInpIt) + ".py"

                    outputFile = File(file_template)
                    jobTask[jobTaskIt].uses(outputFile, link=Link.INPUT)
